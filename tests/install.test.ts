...
test("installs, backs up shell config, and configures PATH once with correctly quoted paths", async () => {
  const root = await mkdtemp(join(tmpdir(), "git-workflow-install-test-"));
  try {
    ...
    expect(await readFile(`${shellConfigFile}.git-workflow.bak`, "utf8")).toBe(original);
    ...
    const output = execFileSync("/bin/zsh", [
      "-f", "-c", 'source "$1"; source "$1"; print -r -- "$PATH"; print -r -- "$GIT_WORKFLOW_TEST_VALUE"; git-workflow',
      "git-workflow-install-test", shellConfigFile,
...
test("creates a missing shell config and rejects broken managed markers before installing", async () => {
  ...
  try {
    ...
    expect(await readFile(shellConfigFile, "utf8")).toContain("# >>> git-workflow >>>");
    const broken = "# >>> git-workflow >>>\nunfinished";
...
test("migrates the old PATH block while preserving unrelated shell configuration", async () => {
  const root = await mkdtemp(join(tmpdir(), "git-workflow-migration-test-"));
  try {
    const source = join(root, "source");
    ...
    const after = "\n# other application\nexport GIT_WORKFLOW_TEST_VALUE=preserved\n";
    await writeFile(source, "#!/bin/sh\nexit 0\n", { mode: 0o755 });
    await writeFile(shellConfigFile, before + "# >>> git_workflow >>>\n# old PATH block\n# <<< git_workflow <<<" + after);
    const options = { binDirectory: join(root, "bin"), shellConfigFile };
    const installed = await installExecutable(source, options);
    expect(installed.executablePath).toBe(join(root, "bin", "git-workflow"));
    ...
    expect(config).not.toContain("git_workflow");
    expect(config.split("# >>> git-workflow >>>")).toHaveLength(2);
